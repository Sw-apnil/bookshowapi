...
const releaseSeatsAndDeleteBooking = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    await step.run("check-payment-status", async () => {
      ...
      // 🛡️ SAFETY: booking already deleted / not found
      if (!booking) {
        return { skipped: true, reason: "booking_not_found" };
      }

      // 2️⃣ If payment already done, do nothing
      if (booking.isPaid) {
        return { skipped: true, reason: "already_paid" };
      }

      // 3️⃣ Fetch show safely
      const show = await Show.findById(booking.show);
      if (!show) {
        return { skipped: true, reason: "show_not_found" };
      }

      // 4️⃣ Release occupied seats
...
const sendBookingConfirmationEmail = inngest.createFunction(
  ...
  async ({ event }) => {
    ...
    if (!user) {
      throw new Error(`User ${booking.user} not found`);
    }

    await sendEmail({
      to: user.email,
      subject: `Payment Confirmation: "${booking.show.movie.title}" booked!`,
      body: `<h2>Hi ${user.name}</h2><p>Your booking is confirmed.</p>`,
    });
  }
);
...
// export the future inngest functions
